...
const BLOCKED_PATHS = [
  // [/^src\/utils\//, 'Utilities belong inside a layer: src/server/lib/ or src/frontend/lib/'],
  // [/^src\/components\//, 'Components belong in src/frontend/components/'],
];
...
const ROUTES = [
  // Anything that touches the game at runtime shares the Harmony/[MBCallback] rules.
  [/^(Patches|Behaviours)\/.*\.cs$/, 'docs/agent/harmony-patches.md'],
  [/^Diagnostics\.cs$/, 'docs/agent/diagnostics.md'],
  [/^Settings\.cs$/, 'docs/agent/mcm-settings.md'],
  // A patch that logs needs the log's population gotchas too (specific → lands last).
  [/^Patches\/.*\.cs$/, 'docs/agent/diagnostics.md'],
];
...
function main() {
  ...
  // Pass 1: structure check — only for new files under src/
  if (isNewFile && filePath.startsWith('src/')) {
    ...
    if (VALID_TOP_DIRS.size > 0) {
      const parts = filePath.split('/');
      const topDir = parts[1]; // src/<topDir>/...
      if (topDir && !VALID_TOP_DIRS.has(topDir)) {
        console.error(
          `[inject-context] Unknown top-level dir: src/${topDir}/\n` +
          `Valid dirs: ${[...VALID_TOP_DIRS].join(', ')}`
        );
        process.exit(2);
      }
    }
